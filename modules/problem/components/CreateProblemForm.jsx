...
const CreateProblemForm = () => {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      //☺️ Rename referenceSolutions to referenceSolution for API compatibility
      const payload = {
        ...values,
        referenceSolution: values.referenceSolutions,
      };
      delete payload.referenceSolutions;
      ...
      if (!response.ok) {
        // Handle validation failure from Judge0
        if (data.testCases) {
          setValidationError({
            type: "validation",
            message: data.message || "Test case validation failed",
            details: data.testCases,
          });
          toast.error("❌ " + (data.message || "Test validation failed"));
          return;
        }
        ...
        if (response.status === 401) {
          setSubmitError({
            type: "authorization",
            message:
              data.message || "You are not authorized to create problems",
          });
        ...
        if (response.status === 400) {
          ...
          toast.error("⚠️ " + (data.message || "Invalid input data"));
          return;
        }
        ...
        if (response.status === 500) {
          setSubmitError({
            type: "server",
            message: data.message || "Failed to save problem in database",
          });
        ...
        setSubmitError({
          type: "unknown",
          message: data.message || "Failed to create problem",
        });
            ...
            <Alert className="mb-6 border-red-200 bg-red-50 dark:bg-red-950">
              <AlertDescription className="text-red-800 dark:text-red-200">
                <div className="font-semibold mb-2">
                  {submitError.type === "authorization" &&
                    "🔒 Authorization Error"}
                  {submitError.type === "validation" && "⚠️ Validation Error"}
                  {submitError.type === "network" && "🌐 Network Error"}
                  {submitError.type === "server" && "🔥 Server Error"}
                  {submitError.type === "parsing" && "❌ Response Error"}
                  {submitError.type === "unknown" && "❌ Error"}
                </div>
          ...
          {validationError && (
            <Alert className="mb-6 border-orange-200 bg-orange-50 dark:bg-orange-950">
              <AlertDescription className="text-orange-800 dark:text-orange-200">
                ...
                <p className="mb-3 font-medium">{validationError.message}</p>
                {validationError.details && (
                  <div className="bg-white dark:bg-slate-900 p-3 rounded-md space-y-2 text-sm">
                    <div>
                      <span className="font-semibold">Input:</span>
                      <p className="font-mono bg-slate-100 dark:bg-slate-800 p-2 rounded mt-1 break-all">
                        {validationError.details.input}
                      </p>
                    </div>
                    <div>
                      <span className="font-semibold">Expected Output:</span>
                      <p className="font-mono bg-slate-100 dark:bg-slate-800 p-2 rounded mt-1 break-all">
                        {validationError.details.expectedOutput}
                      </p>
                    </div>
                    <div>
                      <span className="font-semibold">Actual Output:</span>
                      <p className="font-mono bg-slate-100 dark:bg-slate-800 p-2 rounded mt-1 break-all">
                        {validationError.details.actualOutput || "No output"}
                      </p>
                    </div>
                    {validationError.details.errorMessage && (
                      <div>
                        <span className="font-semibold">Error Message:</span>
                        <p className="font-mono bg-red-100 dark:bg-red-900 p-2 rounded mt-1 break-all">
                          {validationError.details.errorMessage}
                        </p>
                      </div>
                    )}
